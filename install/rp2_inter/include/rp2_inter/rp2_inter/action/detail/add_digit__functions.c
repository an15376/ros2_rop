// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from rp2_inter:action/AddDigit.idl
// generated code does not contain a copyright notice
#include "rp2_inter/action/detail/add_digit__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `uid`
#include "rosidl_runtime_c/string_functions.h"

bool
rp2_inter__action__AddDigit_Goal__init(rp2_inter__action__AddDigit_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // uid
  if (!rosidl_runtime_c__String__init(&msg->uid)) {
    rp2_inter__action__AddDigit_Goal__fini(msg);
    return false;
  }
  return true;
}

void
rp2_inter__action__AddDigit_Goal__fini(rp2_inter__action__AddDigit_Goal * msg)
{
  if (!msg) {
    return;
  }
  // uid
  rosidl_runtime_c__String__fini(&msg->uid);
}

bool
rp2_inter__action__AddDigit_Goal__are_equal(const rp2_inter__action__AddDigit_Goal * lhs, const rp2_inter__action__AddDigit_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // uid
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->uid), &(rhs->uid)))
  {
    return false;
  }
  return true;
}

bool
rp2_inter__action__AddDigit_Goal__copy(
  const rp2_inter__action__AddDigit_Goal * input,
  rp2_inter__action__AddDigit_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // uid
  if (!rosidl_runtime_c__String__copy(
      &(input->uid), &(output->uid)))
  {
    return false;
  }
  return true;
}

rp2_inter__action__AddDigit_Goal *
rp2_inter__action__AddDigit_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_Goal * msg = (rp2_inter__action__AddDigit_Goal *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rp2_inter__action__AddDigit_Goal));
  bool success = rp2_inter__action__AddDigit_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rp2_inter__action__AddDigit_Goal__destroy(rp2_inter__action__AddDigit_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rp2_inter__action__AddDigit_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rp2_inter__action__AddDigit_Goal__Sequence__init(rp2_inter__action__AddDigit_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_Goal * data = NULL;

  if (size) {
    data = (rp2_inter__action__AddDigit_Goal *)allocator.zero_allocate(size, sizeof(rp2_inter__action__AddDigit_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rp2_inter__action__AddDigit_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rp2_inter__action__AddDigit_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rp2_inter__action__AddDigit_Goal__Sequence__fini(rp2_inter__action__AddDigit_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rp2_inter__action__AddDigit_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rp2_inter__action__AddDigit_Goal__Sequence *
rp2_inter__action__AddDigit_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_Goal__Sequence * array = (rp2_inter__action__AddDigit_Goal__Sequence *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rp2_inter__action__AddDigit_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rp2_inter__action__AddDigit_Goal__Sequence__destroy(rp2_inter__action__AddDigit_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rp2_inter__action__AddDigit_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rp2_inter__action__AddDigit_Goal__Sequence__are_equal(const rp2_inter__action__AddDigit_Goal__Sequence * lhs, const rp2_inter__action__AddDigit_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rp2_inter__action__AddDigit_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rp2_inter__action__AddDigit_Goal__Sequence__copy(
  const rp2_inter__action__AddDigit_Goal__Sequence * input,
  rp2_inter__action__AddDigit_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rp2_inter__action__AddDigit_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rp2_inter__action__AddDigit_Goal * data =
      (rp2_inter__action__AddDigit_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rp2_inter__action__AddDigit_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rp2_inter__action__AddDigit_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rp2_inter__action__AddDigit_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result_uid`
#include "rosidl_runtime_c/primitives_sequence_functions.h"

bool
rp2_inter__action__AddDigit_Result__init(rp2_inter__action__AddDigit_Result * msg)
{
  if (!msg) {
    return false;
  }
  // result_uid
  if (!rosidl_runtime_c__int32__Sequence__init(&msg->result_uid, 0)) {
    rp2_inter__action__AddDigit_Result__fini(msg);
    return false;
  }
  return true;
}

void
rp2_inter__action__AddDigit_Result__fini(rp2_inter__action__AddDigit_Result * msg)
{
  if (!msg) {
    return;
  }
  // result_uid
  rosidl_runtime_c__int32__Sequence__fini(&msg->result_uid);
}

bool
rp2_inter__action__AddDigit_Result__are_equal(const rp2_inter__action__AddDigit_Result * lhs, const rp2_inter__action__AddDigit_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // result_uid
  if (!rosidl_runtime_c__int32__Sequence__are_equal(
      &(lhs->result_uid), &(rhs->result_uid)))
  {
    return false;
  }
  return true;
}

bool
rp2_inter__action__AddDigit_Result__copy(
  const rp2_inter__action__AddDigit_Result * input,
  rp2_inter__action__AddDigit_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // result_uid
  if (!rosidl_runtime_c__int32__Sequence__copy(
      &(input->result_uid), &(output->result_uid)))
  {
    return false;
  }
  return true;
}

rp2_inter__action__AddDigit_Result *
rp2_inter__action__AddDigit_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_Result * msg = (rp2_inter__action__AddDigit_Result *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rp2_inter__action__AddDigit_Result));
  bool success = rp2_inter__action__AddDigit_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rp2_inter__action__AddDigit_Result__destroy(rp2_inter__action__AddDigit_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rp2_inter__action__AddDigit_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rp2_inter__action__AddDigit_Result__Sequence__init(rp2_inter__action__AddDigit_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_Result * data = NULL;

  if (size) {
    data = (rp2_inter__action__AddDigit_Result *)allocator.zero_allocate(size, sizeof(rp2_inter__action__AddDigit_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rp2_inter__action__AddDigit_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rp2_inter__action__AddDigit_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rp2_inter__action__AddDigit_Result__Sequence__fini(rp2_inter__action__AddDigit_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rp2_inter__action__AddDigit_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rp2_inter__action__AddDigit_Result__Sequence *
rp2_inter__action__AddDigit_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_Result__Sequence * array = (rp2_inter__action__AddDigit_Result__Sequence *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rp2_inter__action__AddDigit_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rp2_inter__action__AddDigit_Result__Sequence__destroy(rp2_inter__action__AddDigit_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rp2_inter__action__AddDigit_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rp2_inter__action__AddDigit_Result__Sequence__are_equal(const rp2_inter__action__AddDigit_Result__Sequence * lhs, const rp2_inter__action__AddDigit_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rp2_inter__action__AddDigit_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rp2_inter__action__AddDigit_Result__Sequence__copy(
  const rp2_inter__action__AddDigit_Result__Sequence * input,
  rp2_inter__action__AddDigit_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rp2_inter__action__AddDigit_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rp2_inter__action__AddDigit_Result * data =
      (rp2_inter__action__AddDigit_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rp2_inter__action__AddDigit_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rp2_inter__action__AddDigit_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rp2_inter__action__AddDigit_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `intermediate_uid`
// already included above
// #include "rosidl_runtime_c/primitives_sequence_functions.h"

bool
rp2_inter__action__AddDigit_Feedback__init(rp2_inter__action__AddDigit_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // intermediate_uid
  if (!rosidl_runtime_c__int32__Sequence__init(&msg->intermediate_uid, 0)) {
    rp2_inter__action__AddDigit_Feedback__fini(msg);
    return false;
  }
  return true;
}

void
rp2_inter__action__AddDigit_Feedback__fini(rp2_inter__action__AddDigit_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // intermediate_uid
  rosidl_runtime_c__int32__Sequence__fini(&msg->intermediate_uid);
}

bool
rp2_inter__action__AddDigit_Feedback__are_equal(const rp2_inter__action__AddDigit_Feedback * lhs, const rp2_inter__action__AddDigit_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // intermediate_uid
  if (!rosidl_runtime_c__int32__Sequence__are_equal(
      &(lhs->intermediate_uid), &(rhs->intermediate_uid)))
  {
    return false;
  }
  return true;
}

bool
rp2_inter__action__AddDigit_Feedback__copy(
  const rp2_inter__action__AddDigit_Feedback * input,
  rp2_inter__action__AddDigit_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // intermediate_uid
  if (!rosidl_runtime_c__int32__Sequence__copy(
      &(input->intermediate_uid), &(output->intermediate_uid)))
  {
    return false;
  }
  return true;
}

rp2_inter__action__AddDigit_Feedback *
rp2_inter__action__AddDigit_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_Feedback * msg = (rp2_inter__action__AddDigit_Feedback *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rp2_inter__action__AddDigit_Feedback));
  bool success = rp2_inter__action__AddDigit_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rp2_inter__action__AddDigit_Feedback__destroy(rp2_inter__action__AddDigit_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rp2_inter__action__AddDigit_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rp2_inter__action__AddDigit_Feedback__Sequence__init(rp2_inter__action__AddDigit_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_Feedback * data = NULL;

  if (size) {
    data = (rp2_inter__action__AddDigit_Feedback *)allocator.zero_allocate(size, sizeof(rp2_inter__action__AddDigit_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rp2_inter__action__AddDigit_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rp2_inter__action__AddDigit_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rp2_inter__action__AddDigit_Feedback__Sequence__fini(rp2_inter__action__AddDigit_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rp2_inter__action__AddDigit_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rp2_inter__action__AddDigit_Feedback__Sequence *
rp2_inter__action__AddDigit_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_Feedback__Sequence * array = (rp2_inter__action__AddDigit_Feedback__Sequence *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rp2_inter__action__AddDigit_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rp2_inter__action__AddDigit_Feedback__Sequence__destroy(rp2_inter__action__AddDigit_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rp2_inter__action__AddDigit_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rp2_inter__action__AddDigit_Feedback__Sequence__are_equal(const rp2_inter__action__AddDigit_Feedback__Sequence * lhs, const rp2_inter__action__AddDigit_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rp2_inter__action__AddDigit_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rp2_inter__action__AddDigit_Feedback__Sequence__copy(
  const rp2_inter__action__AddDigit_Feedback__Sequence * input,
  rp2_inter__action__AddDigit_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rp2_inter__action__AddDigit_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rp2_inter__action__AddDigit_Feedback * data =
      (rp2_inter__action__AddDigit_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rp2_inter__action__AddDigit_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rp2_inter__action__AddDigit_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rp2_inter__action__AddDigit_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "rp2_inter/action/detail/add_digit__functions.h"

bool
rp2_inter__action__AddDigit_SendGoal_Request__init(rp2_inter__action__AddDigit_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    rp2_inter__action__AddDigit_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!rp2_inter__action__AddDigit_Goal__init(&msg->goal)) {
    rp2_inter__action__AddDigit_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
rp2_inter__action__AddDigit_SendGoal_Request__fini(rp2_inter__action__AddDigit_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  rp2_inter__action__AddDigit_Goal__fini(&msg->goal);
}

bool
rp2_inter__action__AddDigit_SendGoal_Request__are_equal(const rp2_inter__action__AddDigit_SendGoal_Request * lhs, const rp2_inter__action__AddDigit_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!rp2_inter__action__AddDigit_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
rp2_inter__action__AddDigit_SendGoal_Request__copy(
  const rp2_inter__action__AddDigit_SendGoal_Request * input,
  rp2_inter__action__AddDigit_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!rp2_inter__action__AddDigit_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

rp2_inter__action__AddDigit_SendGoal_Request *
rp2_inter__action__AddDigit_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_SendGoal_Request * msg = (rp2_inter__action__AddDigit_SendGoal_Request *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rp2_inter__action__AddDigit_SendGoal_Request));
  bool success = rp2_inter__action__AddDigit_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rp2_inter__action__AddDigit_SendGoal_Request__destroy(rp2_inter__action__AddDigit_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rp2_inter__action__AddDigit_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rp2_inter__action__AddDigit_SendGoal_Request__Sequence__init(rp2_inter__action__AddDigit_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_SendGoal_Request * data = NULL;

  if (size) {
    data = (rp2_inter__action__AddDigit_SendGoal_Request *)allocator.zero_allocate(size, sizeof(rp2_inter__action__AddDigit_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rp2_inter__action__AddDigit_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rp2_inter__action__AddDigit_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rp2_inter__action__AddDigit_SendGoal_Request__Sequence__fini(rp2_inter__action__AddDigit_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rp2_inter__action__AddDigit_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rp2_inter__action__AddDigit_SendGoal_Request__Sequence *
rp2_inter__action__AddDigit_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_SendGoal_Request__Sequence * array = (rp2_inter__action__AddDigit_SendGoal_Request__Sequence *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rp2_inter__action__AddDigit_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rp2_inter__action__AddDigit_SendGoal_Request__Sequence__destroy(rp2_inter__action__AddDigit_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rp2_inter__action__AddDigit_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rp2_inter__action__AddDigit_SendGoal_Request__Sequence__are_equal(const rp2_inter__action__AddDigit_SendGoal_Request__Sequence * lhs, const rp2_inter__action__AddDigit_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rp2_inter__action__AddDigit_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rp2_inter__action__AddDigit_SendGoal_Request__Sequence__copy(
  const rp2_inter__action__AddDigit_SendGoal_Request__Sequence * input,
  rp2_inter__action__AddDigit_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rp2_inter__action__AddDigit_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rp2_inter__action__AddDigit_SendGoal_Request * data =
      (rp2_inter__action__AddDigit_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rp2_inter__action__AddDigit_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rp2_inter__action__AddDigit_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rp2_inter__action__AddDigit_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
rp2_inter__action__AddDigit_SendGoal_Response__init(rp2_inter__action__AddDigit_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    rp2_inter__action__AddDigit_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
rp2_inter__action__AddDigit_SendGoal_Response__fini(rp2_inter__action__AddDigit_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
rp2_inter__action__AddDigit_SendGoal_Response__are_equal(const rp2_inter__action__AddDigit_SendGoal_Response * lhs, const rp2_inter__action__AddDigit_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
rp2_inter__action__AddDigit_SendGoal_Response__copy(
  const rp2_inter__action__AddDigit_SendGoal_Response * input,
  rp2_inter__action__AddDigit_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

rp2_inter__action__AddDigit_SendGoal_Response *
rp2_inter__action__AddDigit_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_SendGoal_Response * msg = (rp2_inter__action__AddDigit_SendGoal_Response *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rp2_inter__action__AddDigit_SendGoal_Response));
  bool success = rp2_inter__action__AddDigit_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rp2_inter__action__AddDigit_SendGoal_Response__destroy(rp2_inter__action__AddDigit_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rp2_inter__action__AddDigit_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rp2_inter__action__AddDigit_SendGoal_Response__Sequence__init(rp2_inter__action__AddDigit_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_SendGoal_Response * data = NULL;

  if (size) {
    data = (rp2_inter__action__AddDigit_SendGoal_Response *)allocator.zero_allocate(size, sizeof(rp2_inter__action__AddDigit_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rp2_inter__action__AddDigit_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rp2_inter__action__AddDigit_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rp2_inter__action__AddDigit_SendGoal_Response__Sequence__fini(rp2_inter__action__AddDigit_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rp2_inter__action__AddDigit_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rp2_inter__action__AddDigit_SendGoal_Response__Sequence *
rp2_inter__action__AddDigit_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_SendGoal_Response__Sequence * array = (rp2_inter__action__AddDigit_SendGoal_Response__Sequence *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rp2_inter__action__AddDigit_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rp2_inter__action__AddDigit_SendGoal_Response__Sequence__destroy(rp2_inter__action__AddDigit_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rp2_inter__action__AddDigit_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rp2_inter__action__AddDigit_SendGoal_Response__Sequence__are_equal(const rp2_inter__action__AddDigit_SendGoal_Response__Sequence * lhs, const rp2_inter__action__AddDigit_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rp2_inter__action__AddDigit_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rp2_inter__action__AddDigit_SendGoal_Response__Sequence__copy(
  const rp2_inter__action__AddDigit_SendGoal_Response__Sequence * input,
  rp2_inter__action__AddDigit_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rp2_inter__action__AddDigit_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rp2_inter__action__AddDigit_SendGoal_Response * data =
      (rp2_inter__action__AddDigit_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rp2_inter__action__AddDigit_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rp2_inter__action__AddDigit_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rp2_inter__action__AddDigit_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
rp2_inter__action__AddDigit_GetResult_Request__init(rp2_inter__action__AddDigit_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    rp2_inter__action__AddDigit_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
rp2_inter__action__AddDigit_GetResult_Request__fini(rp2_inter__action__AddDigit_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
rp2_inter__action__AddDigit_GetResult_Request__are_equal(const rp2_inter__action__AddDigit_GetResult_Request * lhs, const rp2_inter__action__AddDigit_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
rp2_inter__action__AddDigit_GetResult_Request__copy(
  const rp2_inter__action__AddDigit_GetResult_Request * input,
  rp2_inter__action__AddDigit_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

rp2_inter__action__AddDigit_GetResult_Request *
rp2_inter__action__AddDigit_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_GetResult_Request * msg = (rp2_inter__action__AddDigit_GetResult_Request *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rp2_inter__action__AddDigit_GetResult_Request));
  bool success = rp2_inter__action__AddDigit_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rp2_inter__action__AddDigit_GetResult_Request__destroy(rp2_inter__action__AddDigit_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rp2_inter__action__AddDigit_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rp2_inter__action__AddDigit_GetResult_Request__Sequence__init(rp2_inter__action__AddDigit_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_GetResult_Request * data = NULL;

  if (size) {
    data = (rp2_inter__action__AddDigit_GetResult_Request *)allocator.zero_allocate(size, sizeof(rp2_inter__action__AddDigit_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rp2_inter__action__AddDigit_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rp2_inter__action__AddDigit_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rp2_inter__action__AddDigit_GetResult_Request__Sequence__fini(rp2_inter__action__AddDigit_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rp2_inter__action__AddDigit_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rp2_inter__action__AddDigit_GetResult_Request__Sequence *
rp2_inter__action__AddDigit_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_GetResult_Request__Sequence * array = (rp2_inter__action__AddDigit_GetResult_Request__Sequence *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rp2_inter__action__AddDigit_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rp2_inter__action__AddDigit_GetResult_Request__Sequence__destroy(rp2_inter__action__AddDigit_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rp2_inter__action__AddDigit_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rp2_inter__action__AddDigit_GetResult_Request__Sequence__are_equal(const rp2_inter__action__AddDigit_GetResult_Request__Sequence * lhs, const rp2_inter__action__AddDigit_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rp2_inter__action__AddDigit_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rp2_inter__action__AddDigit_GetResult_Request__Sequence__copy(
  const rp2_inter__action__AddDigit_GetResult_Request__Sequence * input,
  rp2_inter__action__AddDigit_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rp2_inter__action__AddDigit_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rp2_inter__action__AddDigit_GetResult_Request * data =
      (rp2_inter__action__AddDigit_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rp2_inter__action__AddDigit_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rp2_inter__action__AddDigit_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rp2_inter__action__AddDigit_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "rp2_inter/action/detail/add_digit__functions.h"

bool
rp2_inter__action__AddDigit_GetResult_Response__init(rp2_inter__action__AddDigit_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!rp2_inter__action__AddDigit_Result__init(&msg->result)) {
    rp2_inter__action__AddDigit_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
rp2_inter__action__AddDigit_GetResult_Response__fini(rp2_inter__action__AddDigit_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  rp2_inter__action__AddDigit_Result__fini(&msg->result);
}

bool
rp2_inter__action__AddDigit_GetResult_Response__are_equal(const rp2_inter__action__AddDigit_GetResult_Response * lhs, const rp2_inter__action__AddDigit_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!rp2_inter__action__AddDigit_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
rp2_inter__action__AddDigit_GetResult_Response__copy(
  const rp2_inter__action__AddDigit_GetResult_Response * input,
  rp2_inter__action__AddDigit_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!rp2_inter__action__AddDigit_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

rp2_inter__action__AddDigit_GetResult_Response *
rp2_inter__action__AddDigit_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_GetResult_Response * msg = (rp2_inter__action__AddDigit_GetResult_Response *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rp2_inter__action__AddDigit_GetResult_Response));
  bool success = rp2_inter__action__AddDigit_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rp2_inter__action__AddDigit_GetResult_Response__destroy(rp2_inter__action__AddDigit_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rp2_inter__action__AddDigit_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rp2_inter__action__AddDigit_GetResult_Response__Sequence__init(rp2_inter__action__AddDigit_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_GetResult_Response * data = NULL;

  if (size) {
    data = (rp2_inter__action__AddDigit_GetResult_Response *)allocator.zero_allocate(size, sizeof(rp2_inter__action__AddDigit_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rp2_inter__action__AddDigit_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rp2_inter__action__AddDigit_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rp2_inter__action__AddDigit_GetResult_Response__Sequence__fini(rp2_inter__action__AddDigit_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rp2_inter__action__AddDigit_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rp2_inter__action__AddDigit_GetResult_Response__Sequence *
rp2_inter__action__AddDigit_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_GetResult_Response__Sequence * array = (rp2_inter__action__AddDigit_GetResult_Response__Sequence *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rp2_inter__action__AddDigit_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rp2_inter__action__AddDigit_GetResult_Response__Sequence__destroy(rp2_inter__action__AddDigit_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rp2_inter__action__AddDigit_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rp2_inter__action__AddDigit_GetResult_Response__Sequence__are_equal(const rp2_inter__action__AddDigit_GetResult_Response__Sequence * lhs, const rp2_inter__action__AddDigit_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rp2_inter__action__AddDigit_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rp2_inter__action__AddDigit_GetResult_Response__Sequence__copy(
  const rp2_inter__action__AddDigit_GetResult_Response__Sequence * input,
  rp2_inter__action__AddDigit_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rp2_inter__action__AddDigit_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rp2_inter__action__AddDigit_GetResult_Response * data =
      (rp2_inter__action__AddDigit_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rp2_inter__action__AddDigit_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rp2_inter__action__AddDigit_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rp2_inter__action__AddDigit_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "rp2_inter/action/detail/add_digit__functions.h"

bool
rp2_inter__action__AddDigit_FeedbackMessage__init(rp2_inter__action__AddDigit_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    rp2_inter__action__AddDigit_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!rp2_inter__action__AddDigit_Feedback__init(&msg->feedback)) {
    rp2_inter__action__AddDigit_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
rp2_inter__action__AddDigit_FeedbackMessage__fini(rp2_inter__action__AddDigit_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  rp2_inter__action__AddDigit_Feedback__fini(&msg->feedback);
}

bool
rp2_inter__action__AddDigit_FeedbackMessage__are_equal(const rp2_inter__action__AddDigit_FeedbackMessage * lhs, const rp2_inter__action__AddDigit_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!rp2_inter__action__AddDigit_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
rp2_inter__action__AddDigit_FeedbackMessage__copy(
  const rp2_inter__action__AddDigit_FeedbackMessage * input,
  rp2_inter__action__AddDigit_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!rp2_inter__action__AddDigit_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

rp2_inter__action__AddDigit_FeedbackMessage *
rp2_inter__action__AddDigit_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_FeedbackMessage * msg = (rp2_inter__action__AddDigit_FeedbackMessage *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rp2_inter__action__AddDigit_FeedbackMessage));
  bool success = rp2_inter__action__AddDigit_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rp2_inter__action__AddDigit_FeedbackMessage__destroy(rp2_inter__action__AddDigit_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rp2_inter__action__AddDigit_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rp2_inter__action__AddDigit_FeedbackMessage__Sequence__init(rp2_inter__action__AddDigit_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_FeedbackMessage * data = NULL;

  if (size) {
    data = (rp2_inter__action__AddDigit_FeedbackMessage *)allocator.zero_allocate(size, sizeof(rp2_inter__action__AddDigit_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rp2_inter__action__AddDigit_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rp2_inter__action__AddDigit_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rp2_inter__action__AddDigit_FeedbackMessage__Sequence__fini(rp2_inter__action__AddDigit_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rp2_inter__action__AddDigit_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rp2_inter__action__AddDigit_FeedbackMessage__Sequence *
rp2_inter__action__AddDigit_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rp2_inter__action__AddDigit_FeedbackMessage__Sequence * array = (rp2_inter__action__AddDigit_FeedbackMessage__Sequence *)allocator.allocate(sizeof(rp2_inter__action__AddDigit_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rp2_inter__action__AddDigit_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rp2_inter__action__AddDigit_FeedbackMessage__Sequence__destroy(rp2_inter__action__AddDigit_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rp2_inter__action__AddDigit_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rp2_inter__action__AddDigit_FeedbackMessage__Sequence__are_equal(const rp2_inter__action__AddDigit_FeedbackMessage__Sequence * lhs, const rp2_inter__action__AddDigit_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rp2_inter__action__AddDigit_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rp2_inter__action__AddDigit_FeedbackMessage__Sequence__copy(
  const rp2_inter__action__AddDigit_FeedbackMessage__Sequence * input,
  rp2_inter__action__AddDigit_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rp2_inter__action__AddDigit_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rp2_inter__action__AddDigit_FeedbackMessage * data =
      (rp2_inter__action__AddDigit_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rp2_inter__action__AddDigit_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rp2_inter__action__AddDigit_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rp2_inter__action__AddDigit_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
